...
#include "exec/container/inside/setup.h"
#include "exec/proto/container.pb.h"
#include "glog/logging.h"
#include "glog/raw_logging.h"
#include "util/cpp/files.h"

using omogen::exec::proto::ContainerExecution;
using omogen::exec::proto::ContainerTermination;
...
int main(int argc, char** argv) {
  ...
  while (true) {
    ...
    ContainerTermination response = omogen::exec::execute(request);
    LOG(INFO) << "Done with termination " << response.DebugString();
    string responseBytes;
    response.SerializeToString(&responseBytes);
    WriteIntToFd(responseBytes.size(), 101);
    WriteToFd(101, responseBytes);
  }
